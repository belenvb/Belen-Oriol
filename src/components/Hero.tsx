...
export function Hero({lang,onNavigate}: HeroProps) {
 return <section id="hero" className="editorial-hero">
  ...
  <div className="hero-bottom"><span>{lang === 'es' ? 'Un lugar. Toda nuestra gente.' : 'One place. All our favourite people.'}</span><button onClick={()=>onNavigate('story')}>{lang === 'es' ? 'Descubrir' : 'Discover'} ↓</button></div>
 </section>;
}
