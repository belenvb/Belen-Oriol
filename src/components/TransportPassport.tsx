...
export function TransportPassport({ lang }: { lang: Language }) {
  ...
  const pages = useMemo(() => {
    const flights: PageData = {
      ...
      card2Distance: isSpanish
        ? '115 km a Salamanca · ~1 h en coche / 45 min en tren'
        : '115 km / 71 miles to Salamanca · ~1 hr drive / 45 min train',
      card2Text: isSpanish
        ? 'Valladolid puede ser una alternativa cómoda para vuelos desde Barcelona y conexiones nacionales.'
        : 'Valladolid can be a convenient alternative for flights from Barcelona and domestic connections.',
  ...
  const requestSpreadChange = (nextSpread: number) => {
    ...
    finishFlip(safeSpread);
  };

  const openPassport = () => setIsOpen(true);
  ...
  const handleDragEnd = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const isMobileGesture = window.matchMedia('(max-width: 760px)').matches;

    if (isMobileGesture) {
      if (dy < -44 || dx < -54) requestSpreadChange(spreadIndex + 1);
      if (dy > 44 || dx > 54) requestSpreadChange(spreadIndex - 1);
      return;
    }

    if (dx < -54) requestSpreadChange(spreadIndex + 1);
    if (dx > 54) requestSpreadChange(spreadIndex - 1);
  };
  ...
  return (
    <div className={`passport-wrap ${isSpanish ? 'passport-spain' : 'passport-usa'}`}>
            ...
            {flipDirection && (
              <div className={`passport-turning-sheet passport-turning-sheet-${flipDirection}`} aria-hidden="true">
                ...
                <div className="passport-turning-face passport-turning-back">
                  <PassportPageFace page={turningBackPage} isSpanish={isSpanish} compact />
                </div>
              </div>
            )}

            <div className="passport-drag-hint" aria-hidden="true">
              {isSpanish ? 'Desliza para pasar página' : 'Swipe to turn page'}
            </div>
          </div>
      ...
      <div className="passport-controls">
        <button onClick={closePassport} disabled={!isOpen || Boolean(flipDirection)} aria-label={isSpanish ? 'Volver a la portada' : 'Back to cover'}>
          <BookOpen size={16} />
          {isSpanish ? 'Portada' : 'Cover'}
        </button>
        ...
        <div className="passport-pagination">
          {spreads.map((spread, index) => (
            <button
              key={spread.right.id}
              className={index === spreadIndex && isOpen ? 'active' : ''}
        ...
        <div className="passport-controls-right">
          <button onClick={() => requestSpreadChange(spreadIndex - 1)} disabled={!isOpen || spreadIndex === 0 || Boolean(flipDirection)} aria-label={isSpanish ? 'Página anterior' : 'Previous page'}>
            <ChevronLeft size={17} />
            {isSpanish ? 'Anterior' : 'Previous'}
          </button>
          <button onClick={() => requestSpreadChange(spreadIndex + 1)} disabled={!isOpen || spreadIndex === spreads.length - 1 || Boolean(flipDirection)} aria-label={isSpanish ? 'Página siguiente' : 'Next page'}>
