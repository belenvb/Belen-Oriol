...
export function TransportPassport({ lang }: { lang: Language }) {
  ...
  const turnByDirection = (direction: 'next' | 'previous') => {
    requestSpreadChange(spreadIndex + (direction === 'next' ? 1 : -1));
  };
  ...
  const handleDragEnd = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const shouldGoNext = mobile ? dy < -44 || dx < -54 : dx < -54;
    const shouldGoPrevious = mobile ? dy > 44 || dx > 54 : dx > 54;

    if (shouldGoNext) {
      dragTriggeredRef.current = true;
      turnByDirection('next');
    } else if (shouldGoPrevious) {
      dragTriggeredRef.current = true;
      turnByDirection('previous');
    }
  };
      ...
      <div className={`passport-stage ${isOpen ? 'is-open' : 'is-closed'}`}>
        ...
        <div className={`passport-book ${flipDirection ? `is-flipping is-${flipDirection}` : ''}`}>
          ...
          >
            ...
            <div className="passport-drag-hint" aria-hidden="true">{isSpanish ? 'Clic o desliza para pasar página' : 'Click or swipe to turn page'}</div>
          </div>
